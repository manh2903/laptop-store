...
<div class="min-h-screen bg-gray-50/50 p-6 pb-10">
    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
            <h3 class="font-bold text-gray-800">Cập Nhật Danh Mục</h3>
            <a href="{{ route('admin.categories.index') }}" class="text-gray-500 hover:text-indigo-600 text-sm font-medium transition">
                <i class="fas fa-arrow-left mr-1"></i> Quay lại
            </a>
        </div>

        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {{-- CỘT TRÁI --}}
                <div class="space-y-6">
                    {{-- 1. LOẠI DANH MỤC (Chỉ còn Menu & Need) --}}
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-3">Loại danh mục <span class="text-red-500">*</span></label>
                        <div class="grid grid-cols-2 gap-4">
                            <label class="cursor-pointer">
                                <input type="radio" name="loai" value="menu" class="peer sr-only" onchange="updateFormUI()" {{ $category->loai == 'menu' ? 'checked' : '' }}>
                            ...
                            <label class="cursor-pointer">
                                <input type="radio" name="loai" value="need" class="peer sr-only" onchange="updateFormUI()" {{ $category->loai == 'need' ? 'checked' : '' }}>
                                <div class="p-3 text-center border rounded-lg bg-white peer-checked:bg-teal-50 peer-checked:border-teal-500 peer-checked:text-teal-700 hover:bg-gray-50 transition shadow-sm h-full flex flex-col justify-center items-center">
                                    <i class="fas fa-filter block mb-1 text-lg"></i> <span class="font-medium text-sm">Nhu Cầu</span>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="ten_danh_muc" value="{{ old('ten_danh_muc', $category->ten_danh_muc) }}" class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Nhập tên..." required>
                    </div>
                    ...
                    <div id="box_parent" class="hidden">
                        ...
                        <select name="parent_id" id="input_parent" onchange="updateFormUI()" class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500 outline-none bg-white">
                            <option value="0" {{ $category->parent_id == 0 ? 'selected' : '' }}>⭐ Là Danh Mục Gốc (Ông)</option>
                            @foreach($parents as $p)
                                <option value="{{ $p->id }}" {{ $category->parent_id == $p->id ? 'selected' : '' }}>{{ $p->ten_danh_muc }}</option>
                                @foreach($p->children as $child)
                                    @if($child->id != $category->id) {{-- Tránh chọn chính mình làm cha --}}
                                        <option value="{{ $child->id }}" {{ $category->parent_id == $child->id ? 'selected' : '' }}>&nbsp;&nbsp;&nbsp;└── {{ $child->ten_danh_muc }}</option>
                                    @endif
                                @endforeach
                            @endforeach
                        </select>
                ...
                <div class="space-y-6 bg-gray-50/50 p-6 rounded-xl border border-dashed border-gray-200">
                    ...
                    <div>
                        ...
                        <input type="number" name="thu_tu" value="{{ old('thu_tu', $category->thu_tu) }}" class="w-24 border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-indigo-500 outline-none">
                    </div>
                    ...
                    <div id="box_icon" class="hidden">
                        ...
                        <div class="flex shadow-sm">
                            ...
                            <input type="text" name="icon" value="{{ old('icon', $category->icon) }}" class="w-full border border-gray-300 rounded-r-lg p-3 focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="VD: fas fa-laptop">
                        </div>
                        ...
                        <div class="flex items-center justify-center w-full">
                            <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-white hover:bg-indigo-50 transition">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    ...
                                    <p class="text-xs text-gray-500">Click để thay ảnh mới</p>
                                </div>
                                <input type="file" name="hinh_anh" class="hidden" onchange="previewImage(this)">
                            </label>
                        </div>
                        <img id="preview" 
                             src="{{ $category->hinh_anh ? asset('storage/'.$category->hinh_anh) : '' }}" 
                             class="{{ $category->hinh_anh ? '' : 'hidden' }} mt-4 h-20 w-auto rounded border shadow-sm mx-auto p-1 bg-white">
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-gray-100">
                ...
                <div class="flex gap-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <label class="inline-flex items-center cursor-pointer group">
                        <input type="radio" name="trang_thai" value="1" class="w-5 h-5 text-indigo-600 focus:ring-indigo-500 border-gray-300" {{ old('trang_thai', $category->trang_thai) == '1' ? 'checked' : '' }}>
                    ...
                    <label class="inline-flex items-center cursor-pointer group">
                        <input type="radio" name="trang_thai" value="0" class="w-5 h-5 text-red-600 focus:ring-red-500 border-gray-300" {{ old('trang_thai', $category->trang_thai) == '0' ? 'checked' : '' }}>
